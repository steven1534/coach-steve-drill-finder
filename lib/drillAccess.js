...
import previewDrills from "./previewDrills.generated.js";
...
export async function unlock(request, code) {
  ...
  const previewId = previewAccess(normalized);
  if (previewId) {
    if (!Array.isArray(previewDrills) || previewDrills.length < 1) {
      throw new Error("Preview drill payload is unavailable.");
    }
    return {
      code: normalized,
      accessId: previewId,
      role: previewId === "coach" ? "coach" : "player",
      drills: previewDrills,
    };
  }
...
export async function previewDiagnostics(request) {
  ...
  try {
    ...
    return {
      ...
      previewDrillCount: Array.isArray(previewDrills) ? previewDrills.length : 0,
