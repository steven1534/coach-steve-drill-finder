...
import generatedDrills from "./drills.generated.js";
...
function accessMapRaw() {
  const production = String(process.env.DRILL_ACCESS_CODES || "").trim();
  if (production) return production;
  if (process.env.VERCEL_ENV === "preview") {
    return String(process.env.DRILL_PREVIEW_ACCESS_CODES || "").trim();
  }
  return "";
}

function mappedAccess(normalized) {
  const raw = accessMapRaw();
  if (!raw) return null;
...
export async function unlock(request, code) {
  ...
  const configuredMap = accessMapRaw();
  if (configuredMap) {
    const accessId = mappedAccess(normalized);
    if (!accessId) return null;
    if (!Array.isArray(generatedDrills) || generatedDrills.length < 1) {
      throw new Error("Server drill payload is unavailable.");
    }
    return {
      code: normalized,
      accessId,
